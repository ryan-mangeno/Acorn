...
static Acorn::Application* s_Instance = nullptr;
...
static void SetupVulkan(const char** extensions, uint32_t extensions_count)
{
    ...
    // Create Vulkan Instance
    {
        ...
        // 1. Convert C-array to Vector for easy manipulation
        std::vector<const char*> instance_extensions(extensions, extensions + extensions_count);

        // 2. Enumerate available extensions to check for Portability (Required for macOS)
        ...
        bool portability_available = false;
    ...
    // Create Logical Device (with 1 queue)
    {
        ...
        // Enumerate physical device extensions for Portability Subset (macOS)
        uint32_t properties_count;
        ...
        for (const auto& p : properties) {
            if (strcmp(p.extensionName, "VK_KHR_portability_subset") == 0) {
...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    ...
    // Updated CreateOrResizeWindow call (requires ImageUsageFlags now)
...
namespace Acorn {
    ...
    void Application::Init()
    {
        ...
        // NOTE: Manual Font Upload (vkQueueSubmit) is REMOVED.
        // ImGui_ImplVulkan_NewFrame() handles this automatically now.
    }
